...
export class ProductDetailsPage implements OnInit {
      ...
      (res: any) => {
        ...
        if (res.error === false) {
          ...
          for (let i = 0; i < res.result_GetProductDetails.length; i++) {
            ...
            this.slides = res.result_GetProductDetails[i].product_images;
            this.Color = res.result_GetProductDetails[i].product_variations;
  ...
  async goToCart() {
    ...
    if (this.color_id != '') {
      ...
      this.toastService.presentToast('Please Select Color')
    }
  ...
  PhotoView(name, url) {
    this.photoViewer.show('http://purefresh.demobrains.com/public/product_images/' + url, name, { share: true });
  ...
  MakeDefault(event) {
    this.color_id = event.detail.value.color_id;
    console.log("Color Selected =>" + event.detail.value.color_id);
    const data = {
      product_id: this.product_id,
      color_id: this.color_id,
      size_id: ''
    ...
    this.authService.GetProductVariation(data).subscribe(
      (res: any) => {
        console.log(res);
        if (res.error === false) {
          if (res.result_GetProductVariation.images?.length > 0) {
            this.slides = res.result_GetProductVariation.images;
          }

          this.Size = res.result_GetProductVariation.sizes;
          this.Data.price = res.result_GetProductVariation.price;
  ...
  MakeDefaultSize(event) {
    this.color_id = event.detail.value.color_id;
    this.size_id = event.detail.value.size_id;

    console.log("Size Selected =>" + JSON.stringify(event.detail.value.color_id));
    const data = {
      product_id: this.product_id,
      color_id: this.color_id,
      size_id: this.size_id
